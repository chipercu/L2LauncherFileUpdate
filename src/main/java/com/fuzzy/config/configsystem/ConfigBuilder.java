...
import com.fuzzy.CompactJsonFormat.CompactJsonObject;
import com.fuzzy.config.AppConfig;
import com.fuzzy.crypt.Crypto;
...
public class ConfigBuilder<T extends ISubsystemConfig> {
    ...
    private ConfigBuilder(Class<T> configClass, boolean encrypt) throws NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        if (json.isEmpty()) {
            ...
            save(instance.getConfigPathName(), configClass, false);
        } else {
    ...
    private ConfigBuilder(Class<T> configClass, boolean encrypt, boolean update) throws NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        this.encrypt = encrypt;
        initConfigDirectory();
        final T instance = configClass.getDeclaredConstructor().newInstance();
        if (update){
            save(instance.getConfigPathName(), configClass, true);
        }else {
            JSONObject json = readJSON(instance.getConfigPathName());
            if (json.isEmpty()) {
                initDefault(configClass);
    ...
    public static void load(Class<? extends ISubsystemConfig> configClass, boolean encrypt) {
        try {
            new ConfigBuilder<>(configClass, encrypt, false);
    ...
    public static void update(Class<? extends ISubsystemConfig> configClass, boolean encrypt) {
        try {
            new ConfigBuilder<>(configClass, encrypt, true);
        } catch (NoSuchMethodException | InvocationTargetException | InstantiationException |
                 IllegalAccessException e) {
            throw new RuntimeException(e);
        }
    ...
    private void initConfigDirectory() {
        File configPath = new File(AppConfig.CONFIG_DIRECTORY);
    ...
    private void loadFrom(JSONObject json, String configPathName, Class<T> configClass) throws IllegalAccessException {
        ...
        if (need_save) {
            save(configPathName, configClass, false);
        }
    }
    ...
    public void save(String configPathName, Class<T> configClass, boolean update) throws IllegalAccessException {
        ...
        for (Field field : annotatedFields) {
            ...
            if (update){
                ...
                value = configObjectMap.get(field.getName());
            }
    ...
    private void saveToFile(Path path, String json) {
        if (encrypt){
            try {
    ...
    private JSONObject readJSON(String configPathName) {
